...
def homepage(request):
    ...
    posts = (
        Post.objects.filter(visibility__in=allowed_visibilities)
        .order_by("-id")
    ...
    return render(
        request,
        "posts.html",
        dict(posts=posts, user=request.user, full_text=False, active_page="home"),
    )
...
def category(request, category_slug):
    ...
    posts = (
        Post.objects.filter(
            categories__slug=category_slug, visibility__in=allowed_visibilities
        )
        .order_by("-id")
        .prefetch_related("categories")
        .prefetch_related("photos")
...
def post(request, post_slug):
    ...
    if post_obj.visibility in allowed_visibilities:
        return render(
            request,
            "posts.html",
            dict(posts=[post_obj], user=request.user, full_text=True),
        )
